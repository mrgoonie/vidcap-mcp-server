...
export const makeSlug = (input: string, opt: MakeSlugOptions = {}) => {
	...
	if (isEmpty(str) || containsChinese(str) || containsEmoji(str))
		str = 'item';

	// Merge options
	for (const key in defaults) {
		if (!Object.prototype.hasOwnProperty.call(opt, key))
			(opt as any)[key] = (defaults as any)[key];
	}
...
export async function makeUniqueSlug(
	prismaSchema: any,
	input: string,
	opt: MakeSlugOptions & { field: string; atempt: number } = {
		field: 'slug',
		atempt: 0,
	},
) {
	...
	const uniqueSlug = await prismaSchema.findUnique({
		where: { [opt.field]: slug },
	});

	if (uniqueSlug) {
		opt.atempt++;
		return makeUniqueSlug(prismaSchema, `${input}-${opt.atempt}`, {
			...opt,
			field: opt.field,
		});
	}
